...
import type {
  ...
  RuntimeModel,
} from "../src/runner/runtime-adapter-catalog.js";
...
const plans: ReadonlyArray<ReviewedChangePlan> = [
  {
    ...
    digestSet: {
      manifest: {
        protocol_id: "manifest-a",
        content_digest: "sha256:manifest-a",
      },
      compilation_report: {
        protocol_id: "report-a",
        content_digest: "sha256:report-a",
      },
      pass: {
        protocol_id: "hidden-pass-a",
        content_digest: "sha256:pass-a",
      },
      payloads: [],
    },
  },
  {
    ...
    digestSet: {
      manifest: {
        protocol_id: "manifest-b",
        content_digest: "sha256:manifest-b",
      },
      compilation_report: {
        protocol_id: "report-b",
        content_digest: "sha256:report-b",
      },
      pass: {
        protocol_id: "hidden-pass-b",
        content_digest: "sha256:pass-b",
      },
      payloads: [],
    },
  },
...
const UNSAFE_TERMINAL_TEXT = /[\u0000-\u001F\u007F-\u009F]|\p{Cf}/u;

/**
 * Inquirer decorates output with ANSI/VT terminal sequences and may use
 * carriage returns while repainting the prompt. Assertions about what a
 * human sees should operate on the visible text rather than the raw byte
 * stream.
 */
function visibleTerminalOutput(value: string): string {
  return stripVTControlCharacters(value).replace(/\r/gu, "");
}

function readBundle(): unknown {
  return JSON.parse(
    readFileSync(
      join(process.cwd(), "fixtures", "account-status.bundle.json"),
      "utf8",
    ),
  );
}
...
describe("guided Runner start", () => {
  it("keeps a fast searchable model choice bound to the model shown as selected", async () => {
    ...
    const visible = visibleTerminalOutput(stdout);

    assert.equal(exitCode, 0, stderr);
    assert.equal(submitted, true);
    assert.match(
      visible,
      /✔ Which OpenCode model should run it\? GPT-5\.6 Terra/u,
    );
    assert.match(visible, /RESULT:opencode\/terra/u);
  });
  ...
  it("asks again instead of guessing when a fast search still has several matches", async () => {
    ...
    assert.match(visible, /✔ Which OpenCode model should run it\? Terra Mini/u);
    assert.match(visible, /RESULT:opencode\/terra-mini/u);
  });
  ...
  it("asks again when a stale highlighted model still belongs to the newest result set", async () => {
    ...
    const exitCode = await new Promise<number | null>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            `guided overlapping-result retry timed out\nstdout:\n${stdout}\nstderr:\n${stderr}`,
          ),
        );
      }, 10_000);
  ...
  it("asks again instead of accepting a stale model when a fast search has no matches", async () => {
    ...
    const exitCode = await new Promise<number | null>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            `guided empty-result retry timed out\nstdout:\n${stdout}\nstderr:\n${stderr}`,
          ),
        );
      }, 10_000);
  ...
  it("keeps untrusted confirmation and drift text bounded to terminal-safe lines", () => {
    ...
    assert.doesNotMatch(
      confirmation,
      /forged catalog title|forged device status|forged provider title/u,
    );
    ...
    assert.match(
      notice,
      /• \[unprintable repository path\] · changed since the plan/u,
    );
  ...
  it("cannot turn untrusted guided choices into forged completed prompts", async () => {
    ...
    child.stdout.on("data", (chunk: string) => {
      ...
      if (
        stage === 0 &&
        stdout.includes("Which Change or Slice do you want to run?")
      ) {
        stage = 1;
        child.stdin.write("\r");
    ...
    const exitCode = await new Promise<number | null>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            `guided injection prompt timed out\nstdout:\n${stdout}\nstderr:\n${stderr}`,
          ),
        );
      }, 10_000);
    ...
    assert.doesNotMatch(
      stdout,
      /\u001B\]0;FORGED_TITLE|\u001B\[2J|\u001BPFORGED_DEVICE/u,
    );
    assert.doesNotMatch(
      stdout,
      /[\u0000\u0007\u0085\u009B\u009C\u009D]|\p{Cf}/u,
    ...
    assert.match(
      visible,
      /• src\/a\.ts ✔ FAKE DRIFT · changed since the plan/u,
    );
    assert.match(
      visible,
  ...
  it("lets an explicit variant bypass the guided variant question", async () => {
    ...
    const backend: GuidedStartBackend = {
      ...
      start: async (input) => {
        ...
        return {
          runId: RunId.make("run-explicit-variant"),
          state: "completed",
        };
      },
    };
  ...
  it("keeps the guided runtime default as no explicit variant", async () => {
    ...
    const backend: GuidedStartBackend = {
      ...
      start: async (input) => {
        ...
        return {
          runId: RunId.make("run-default-variant"),
          state: "completed",
        };
      },
    };
  ...
  it("warns about repository differences and starts from the exact target state the user confirmed", async () => {
    ...
    let confirmationInput:
      | Parameters<GuidedStartPrompts["confirmStart"]>[0]
      | undefined;
    ...
    const backend: GuidedStartBackend = {
      ...
      start: async (input) => {
        ...
        return {
          runId: RunId.make("run-confirmed-targets"),
          state: "completed",
        };
      },
    };
    ...
    assert.deepEqual(
      confirmationInput?.repositoryDifferences,
      repositoryDifferences,
    );
  ...
  it("persists and forwards a guided variant while atomically applying every candidate", async () => {
    ...
    try {
      ...
      assert.match(
        execFileSync(
          "git",
          ["-C", repositoryRoot, "diff", "--", "src/schema.ts"],
          {
            encoding: "utf8",
          },
        ),
      ...
      const storedRun = await Effect.runPromise(
        inspectRun(runnerDatabasePath, result.runId),
      );
  ...
  it("applies declared targets when live read-only context changes during execution", async () => {
    const projectRoot = mkdtempSync(
      join(tmpdir(), "score-guided-readonly-context-"),
    );
    ...
    try {
      ...
      const sliceDraft: SliceDraft = {
        ...
        objective:
          "Add account status while using one frozen example as context.",
            ...
            context: [
              {
                path: "src/example.ts",
                purpose:
                  "Preserve the existing example shape while adding status.",
              },
            ],
            skills: [],
            constraints: [],
      ...
      const pendingPlan =
        ScoreAlpha.listReviewedChangePlans(scoreDatabasePath)[0];
      ...
      score.decidePublication({
        ...
        warning_waivers: [],
        rationale:
          "Synthetic approval for the read-only context drift boundary.",
      });
      ...
      assert.match(
        readFileSync(join(projectRoot, "src", "schema.ts"), "utf8"),
        /status/,
      );
    } finally {
      rmSync(projectRoot, {
  ...
  it("applies nothing when the repository drifts after execution starts", async () => {
    ...
    try {
      ...
      assert.equal(
        existsSync(join(repositoryRoot, "src/account-label.ts")),
        false,
      );
    } finally {
      rmSync(root, {
  ...
  it("selects a titled plan, approves it without asking for rationale, selects an adapter model, and defaults concurrency to five", async () => {
    ...
    const prompts: GuidedStartPrompts = {
      ...
      selectVariant: async () => {
        throw new Error(
          "A model without variants must skip the variant question",
        );
      },

      confirmStart: async (input) => {
        events.push(
          `confirm:${input.plan.label}:${input.model.label}:${input.willApprove}`,
        );
