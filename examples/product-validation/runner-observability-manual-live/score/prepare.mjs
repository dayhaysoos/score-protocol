...
if (result.status === "invalid") {
  process.stderr.write("SCORE could not prepare the manual live-feed fixture.\n");
  process.stderr.write(`${JSON.stringify(result.findings, null, 2)}\n`);
  process.exitCode = 1;
} else {
  for (const slice of result.slices) {
    if (slice.state === "implemented") {
      process.stdout.write(`✓ ${slice.title} · already applied · Run ${slice.runId}\n`);
    } else if (slice.state === "review_ready") {
      process.stdout.write(`○ ${slice.title} · review ready\n`);
